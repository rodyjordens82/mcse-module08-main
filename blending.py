...
if __name__ == "__main__":
    ...
    # Blending: take the average of the predictions from the individual models
    blended_train_preds = np.mean(combined_train_preds, axis=1)
    blended_test_preds = np.mean(combined_test_preds, axis=1)

    # Calculate metrics for the blended predictions
    print(f"{(time.time() - start_time):.5f}\t Calculating metrics for Blended Model")
    print_and_calc_metrics(y_train, blended_train_preds, "Blended Model Train")
    print_and_calc_metrics(y_test, blended_test_preds, "Blended Model Test")
    ...
    if VALIDATE_SET:
        ...
        # Blending for validation set
        blended_validate_preds = np.mean(combined_validate_preds, axis=1)

        print(
            f"{(time.time() - start_time):.5f}\t Calculating validation metrics for Blended Model"
        )
        print_and_calc_metrics(y_validate, blended_validate_preds, "Blended Model Validate")
